pipeline{
            ...
            steps{
                script{
                    withCredentials([string(credentialsId: 'docker_pass', variable: 'docker_password')]) {
                             sh '''
                                docker build -t malikmuneeb98900/springapp:${VERSION} .
                                docker login --username=malikmuneeb98900 --password=Cypher@12345 
                                docker push  malikmuneeb98900/springapp:${VERSION}
    ...
    post {
        always {
            mail bcc: '', body: "<br>Project: ${env.JOB_NAME} <br>Build Number: ${env.BUILD_NUMBER} <br> URL de build: ${env.BUILD_URL}", cc: '', charset: 'UTF-8', from: '', mimeType: 'text/html', replyTo: '', subject: "${currentBuild.result} CI: Project name -> ${env.JOB_NAME}", to: "malikmuneeb989000@gmail.com";  
         }
       }
}
